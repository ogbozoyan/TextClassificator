...
def pytest_fixture_setup(fixturedef: Any, request: FixtureRequest) -> None:
    ...
    if (
        isasyncgenfunction(func) or iscoroutinefunction(func)
    ) and "anyio_backend" in request.fixturenames:
...
def pytest_pyfunc_call(pyfuncitem: Any) -> Optional[bool]:
    ...
    if backend:
        ...
        if hasattr(pyfuncitem.obj, "hypothesis"):
            ...
            if (
                original_func.__qualname__ != run_with_hypothesis.__qualname__
                and iscoroutinefunction(original_func)
            ):
...
def anyio_backend_name(anyio_backend: Any) -> str:
    return anyio_backend if isinstance(anyio_backend, str) else anyio_backend[0]


@pytest.fixture
def anyio_backend_options(anyio_backend: Any) -> Dict[str, Any]:
    return {} if isinstance(anyio_backend, str) else anyio_backend[1]
